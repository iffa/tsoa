      ...
      describe('should set additionalProperties to false if noImplicitAdditionalProperties is set to "throw-on-extras" (when there are no dictionary or any types)', () => {
        ...
        const assertionsPerProperty: Record<keyof TestModel, (propertyName: string, schema: Swagger.Schema3 | Swagger.Schema31) => void> = {
          ...
          nullableValueRecord: (propertyName, propertySchema) => {
            ...
            // Record<string, string | null> must preserve null in additionalProperties
            // (regression: getIndexInfosOfType strips null when strictNullChecks is off)
            expect(schema).to.be.deep.eq({
              additionalProperties: {
                nullable: true,
                type: 'string',
          ...
          nullableRecord: (propertyName, propertySchema) => {
            // Record<string, string | null> | null — the property itself is also nullable.
            // Should produce allOf[$ref] + nullable:true, where the $ref points to the same
            // schema as nullableValueRecord (additionalProperties must include null).
